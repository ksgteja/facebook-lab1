...
$http.get('/getWorkInfo').success(function(data){
	...
	$scope.currentwork = {};
		if(data.length > 0){
			...
			$scope.currentwork = {company : data[0].company,
								position: data[0].position,
								workYear: data[0].year}
...
$http.get('/getContactInfo').success(function(data){
	...
	data[0].dob = data[0].dob.toString().slice(0,10);
...
$http.get('/getLifeEvents').success(function(data){
	...
	$scope.contactUpdate = function(contact){
		...
		$http.post('/updateContact',{contactInfo : contact}, function(response){
	...
	$scope.updateEducation = function(form, activity){
		...
		if(activity == 'college'){
			...
			$http.post('/insertCollegeInfo',{formData : form}).success(function(data){
